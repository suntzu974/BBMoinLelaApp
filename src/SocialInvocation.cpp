...
void SocialInvocation::childCardDone(const bb::system::CardDoneMessage &doneMessage)
{
	...
	{
		toast->setBody(QString("User Picked up: ") + doneMessage.data());
	}
	toast->show();
}
...
void SocialInvocation::imageToData(const QByteArray &base64) {
    QByteArray decoded = QByteArray::fromBase64(base64);
    QImage qImage;
    qImage.loadFromData(decoded);

    QString imgPath = "shared/photos/social.jpg";
    QFile outFile(imgPath);
    outFile.open(QIODevice::WriteOnly);
    qImage.save(&outFile, "JPEG");
